            ...
            <div class="messages">
                ...
                >
                    <div
                        v-if="
                            idx === 0 ||
                            !isSameDay(message.sentAt, messages[idx - 1].sentAt)
                        "
                    ...
                    >
                        ...
                        <div
                            ...
                            :class="{
                                'message--mine':
                                    message.sender.userId === user.userId,
                                editing: editingMessageId === message.messageId,
                            }"
                            @mouseenter="hoveredMessageId = message.messageId"
                            @mouseleave="hoveredMessageId = null"
                            :ref="(el) => setMessageRef(message.messageId, el)"
                        >
                            <template
                                v-if="
                                    conversation.type === 'group' &&
                            ...
                            <div
                                v-if="editingMessageId === message.messageId"
                            ...
                            >
                                ...
                                >
                                    <div class="message__image-padding">
                                        ...
                                        />
                                        <span
                                            class="text-caption message__image-timestamp"
                                        >
                                            <span
                                            ...
                                            >
                                            <span
                                                class="text-caption__time-and-ticks"
                                            >
                                                {{
                                                    formatTime(
                                                        isEdited(message)
                                ...
                                <button
                                    ...
                                    @click.stop="
                                        openMenu(message.messageId, $event)
                                    "
                                    :ref="
                                        (el) =>
                                            setButtonRef(message.messageId, el)
                                    "
                                >
                                    <svg
                                        viewBox="0 0 24 24"
                                ...
                                <div
                                    v-if="menuOpenFor === message.messageId"
                                    class="message__dropdown-menu"
                                    ref="menuRef"
                                    :style="menuStyles"
                                >
                                    <button @click="commentMessage(message)">
                        ...
                        <div
                            class="comment-list"
                            v-if="message.comments && message.comments.length"
                        >
                            <template
                                v-for="group in groupComments(message.comments)"
                                :key="group.emoji"
                            >
                                <button
                                    class="comment"
                                    :class="{
                                        'my-comment': group.users.includes(
                                            user.userId,
                                        ),
                                    }"
                                    @click="
                                        onEmojiClick(message.messageId, group)
                                    "
                                >
                                    {{ group.emoji }}
                                    <span
...
function groupComments(comments) {
    ...
    for (const comment of comments) {
        const userId = comment.commenter?.userId; // <-- usa questo!
