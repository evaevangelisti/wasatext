<template>
    <div class="new-conversation">
        ...
        <button class="new-group-button" @click="emit('showNewGroup')">
        ...
        <ul class="users">
            <li v-for="user in results" :key="user.userId" class="user">
                <button
                    class="user__button"
                    @click="createPrivateConversation(user.userId)"
                    :disabled="creating"
                >
                    <img
                        :src="
...
const emit = defineEmits(["activeConversation", "showNewGroup"]);
...
async function createPrivateConversation(otherUserId) {
    ...
    try {
        ...
        if (e.response && e.response.status === 409) {
            emit("activeConversation", e.response.data);
        }
    } finally {
