<template>
  <div class="sidebar__header">
    <span class="text-title">WASAText</span>
    <div class="sidebar__icon-list">
      <button
        class="sidebar__icon-button"
        :class="{
          'sidebar__icon-button--active': mode === 'conversationList',
        }"
        @click="$emit('show-conversation-list')"
      >
        <svg viewBox="0 0 24 24" fill="none" class="sidebar__icon">
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M12 3C7.85113 3 4 5.73396 4 10C4 11.5704 4.38842 12.7289 5.08252 13.6554C5.79003 14.5998 6.87746 15.3863 8.41627 16.0908L9.2326 16.4645L8.94868 17.3162C8.54129 18.5384 7.84997 19.6611 7.15156 20.5844C9.56467 19.8263 12.7167 18.6537 14.9453 17.1679C17.1551 15.6948 18.3969 14.5353 19.0991 13.455C19.7758 12.4139 20 11.371 20 10C20 5.73396 16.1489 3 12 3ZM2 10C2 4.26604 7.14887 1 12 1C16.8511 1 22 4.26604 22 10C22 11.629 21.7242 13.0861 20.7759 14.545C19.8531 15.9647 18.3449 17.3052 16.0547 18.8321C13.0781 20.8164 8.76589 22.2232 6.29772 22.9281C5.48665 23.1597 4.84055 22.6838 4.56243 22.1881C4.28848 21.6998 4.22087 20.9454 4.74413 20.3614C5.44439 19.5798 6.21203 18.5732 6.72616 17.4871C5.40034 16.7841 4.29326 15.9376 3.48189 14.8545C2.48785 13.5277 2 11.9296 2 10Z"
            fill="var(--color-secondary)"
          />
        </svg>
      </button>
      <button
        class="sidebar__icon-button"
        :class="{
          'sidebar__icon-button--active':
            mode === 'newConversation' || mode === 'newGroup',
        }"
        @click="$emit('show-new-conversation')"
      >
        <svg viewBox="0 0 24 24" fill="none" class="sidebar__icon">
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M12 3C7.85113 3 4 5.73396 4 10C4 11.5704 4.38842 12.7289 5.08252 13.6554C5.79003 14.5998 6.87746 15.3863 8.41627 16.0908L9.2326 16.4645L8.94868 17.3162C8.54129 18.5384 7.84997 19.6611 7.15156 20.5844C9.56467 19.8263 12.7167 18.6537 14.9453 17.1679C17.1551 15.6948 18.3969 14.5353 19.0991 13.455C19.7758 12.4139 20 11.371 20 10C20 5.73396 16.1489 3 12 3ZM2 10C2 4.26604 7.14887 1 12 1C16.8511 1 22 4.26604 22 10C22 11.629 21.7242 13.0861 20.7759 14.545C19.8531 15.9647 18.3449 17.3052 16.0547 18.8321C13.0781 20.8164 8.76589 22.2232 6.29772 22.9281C5.48665 23.1597 4.84055 22.6838 4.56243 22.1881C4.28848 21.6998 4.22087 20.9454 4.74413 20.3614C5.44439 19.5798 6.21203 18.5732 6.72616 17.4871C5.40034 16.7841 4.29326 15.9376 3.48189 14.8545C2.48785 13.5277 2 11.9296 2 10Z"
            fill="var(--color-secondary)"
          />
          <path
            d="M12 6C11.4477 6 11 6.44771 11 7V9H9C8.44772 9 8 9.44771 8 10C8 10.5523 8.44772 11 9 11H11V13C11 13.5523 11.4477 14 12 14C12.5523 14 13 13.5523 13 13V11H15C15.5523 11 16 10.5523 16 10C16 9.44772 15.5523 9 15 9H13V7C13 6.44771 12.5523 6 12 6Z"
            fill="var(--color-secondary)"
          />
        </svg>
      </button>
      <button
        class="sidebar__icon-button"
        :class="{ 'sidebar__icon-button--active': mode === 'profile' }"
        @click="$emit('show-profile')"
      >
        <img
          v-if="user"
          :src="
            user.profilePicture
            ? backendBaseUrl + user.profilePicture
            : defaultProfilePicture
        "
          alt="Profile"
          class="sidebar__profile-picture"
        >
        <span v-else class="sidebar__profile-picture sidebar__profile-picture--placeholder" />
      </button>
    </div>
  </div>
</template>

<script setup>
import { backendBaseUrl } from "@/services/baseUrl";
import defaultProfilePicture from "@/assets/default-profile-picture.jpg";

const props = defineProps({
  user: Object,
  mode: String,
});

const emit = defineEmits([
  "show-conversation-list",
  "show-new-conversation",
  "show-profile",
]);
</script>

<style scoped>
.sidebar__header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 1rem 1.5rem;
}

.sidebar__icon-list {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 0.5rem;
}

.sidebar__icon-button {
  display: flex;
  align-items: center;
  justify-content: center;
  border: none;
  border-radius: 50%;
  padding: 0.3rem;
  background-color: inherit;
}

.sidebar__icon-button:hover {
  background-color: #252526;
  transition: background 0.1s;
}

.sidebar__icon-button--active > svg > path {
  fill: var(--color-primary);
}

.sidebar__icon-button--active > .sidebar__profile-picture {
  outline: 2px solid var(--color-primary);
}

.sidebar__icon {
  width: 24px;
  height: 24px;
}

.sidebar__profile-picture {
  width: 24px;
  height: 24px;
  border: 2px transparent;
  border-radius: 50%;
  object-fit: cover;
}
</style>
